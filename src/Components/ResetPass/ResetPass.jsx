...
export default function ResetPass() {
  ...
  let validationSchema = yup.object({
    ...
    newPassword: yup.string().required('Password is required').matches(/^[A-Z]\w{4,10}$/, 'Password must be at least 8 characters long with capital one letter ( A ) at least ex:( Mohamed23153 )'),
  })
  ...
  return <>
    <h2 className=' text-center text-main font-bold mt-3'>ResetPass</h2>
    ...
    <div className='h-72'>
      <form className="md:w-1/2 mx-auto pt-4" onSubmit={formik.handleSubmit}>
        ...
        <div className="relative z-0 w-full mb-5 group flex items-center ">
          <input type={passwordHidden ? 'password' : 'text'} name="newPassword" id="newPassword" value={formik.values.newPassword} onChange={formik.handleChange} onBlur={formik.handleBlur} className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-main focus:outline-none focus:ring-0 focus:border-green-500 peer" placeholder=" " />
          ...
          <div><i onClick={() => setPasswordHidden(!passwordHidden)} className={`fa-solid  absolute top-4 right-0 cursor-pointer ${passwordHidden ? 'fa-eye-slash' : 'fa-eye'} ${formik.errors.newPassword && formik.touched.newPassword ? 'text-red-800' :'text-main'}`}></i></div>
        </div>
